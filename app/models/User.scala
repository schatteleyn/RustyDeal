...
object Users {
	...
	def login(username: String, password: String): Future[Option[User]] = {
		val password = MessageDigest.getInstance("SHA-1").digest(password.getBytes)
		...
		bucket.find[User]("user", "by_username")(query).map(_.headOption.filter(_.password == password))
	}
}
